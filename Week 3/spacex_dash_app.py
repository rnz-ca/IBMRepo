...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        return px.pie(filtered_df, values='class', names='Launch Site', title='Launch success rate for all launch sites')
    else:
...
def get_scatter_chart(entered_site, slider):
    ...
    if entered_site == 'ALL':
        return px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Launch success rate for all launch sites')
    else:
        # return the outcomes in pie chart for a selected site
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        filtered_df['outcome'] = filtered_df['class'].apply(lambda x: 'Success' if (x == 1) else 'Failure')
        return px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Launch success rate for ' + entered_site)
